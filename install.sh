...
link_extension()
{
    ...
    ln -sf "${SRCDIR}/${PLUGIN}.py" "${KLIPPER_PATH}/klippy/extras/${PLUGIN}.py"
    echo "[OK]"
}
...
add_updater()
{
    ...
    if [ "$update_section" -eq 0 ]; then
        ...
        echo "origin: ${GITREPO}" >> "$MOONRAKER_CONFIG"
...
uinstall()
{
    if [ -f "${KLIPPER_PATH}/klippy/extras/${PLUGIN}.py" ]; then
        echo -n "Uninstalling ${PLUGIN}... "
        rm -f "${KLIPPER_PATH}/klippy/extras/${PLUGIN}.py"
        rm -f "${KLIPPER_PATH}/klippy/extras/${PLUGIN}.pyc"
        echo "[OK]"
        echo "You can now remove the \"[update_manager ${PLUGIN}]\" section in your moonraker.conf and delete this directory."
        echo "You also need to remove the \"[${PLUGIN}]\" section in your Klipper configuration..."
    else
        echo -n "${KLIPPER_PATH}/klippy/extras/${PLUGIN}.py not found. Is it installed? "
        echo "[FAILED]"
    fi
}
